...
My friends and family have probably had enough of me talking about data, so I decided to use a movie dataset that I found on the internet [here](https://grouplens.org/datasets/movielens/).  I've copied the data to my [github](https://github.com/plb2018/DATA607/tree/master/ml-latest-small) and reference that copy in my code.  

I've opted to do all my MySQL work directly from R, so the only file that i have to show is a .rmd which can be found in my here in my github.  I've also included my output [here] (i.e. the joined table) in as a .csv in case anybody is interested.
...
Next we'll read the movie data into dataframes.  The data are contained in 3 separate files (links, ratings, movies) which we'll end up combining into a single db table later.  
...
```
And finally, we'll pull a sample out of the newly created tabke into an R dataframe:

```{r}

query = dbSendQuery(con, "SELECT * FROM movie_ratings")
data = fetch(query)

data[1:5,]

write.csv(data, file = "DATA_607_WEEK2_OUTPUT.csv")
